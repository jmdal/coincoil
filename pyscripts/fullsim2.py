...
class Coilsim:
    def __init__(self, wdia, bwid, bhei, turn, clen, capa, volt, scon):
        ...
        self.ttll = wirelengthrect(wdia, bwid, bhei, turn, self.tpll) # Total Length
        self.lays = math.ceil(turn/self.tpll) # Layer Count
        self.fwid = self.bwid + self.lays*(2*wdia) # Final Width
        self.fhei = self.bhei + self.lays*(2*wdia) # Final Height
        ...
        self.cind = (turn**2) * (4 * 10**-7)*( -2*(bwid + bhei) + 2*math.sqrt(bwid**2 + bhei**2) - bhei*math.log((bhei + math.sqrt(bwid**2 + bhei**2))/bwid) - bwid*math.log((bwid + math.sqrt(bwid**2 + bhei**2))/bhei) + bhei*math.log(4*bhei/wdia) + bwid*math.log(4*bwid/wdia) ) * 1000 # x1000 correction factor
...
def fullsim(parameters, spice_file=""):
    ...
    while True:
        ...
        if prompt == "C":
            ...
            print("Test [xx]\tWdia [mm]\tBwid [mm]\tBhei [mm]\tTurn [xx]\tClen [mm]\tScon [xx]\tTlen [mm]\tCapa [µF]\tVolt [1V]\tFwid [mm]\tFhei [mm]\tCres [mΩ]\tCind [µH]")
            for i in range(len(parameters)):
                pm = parameters[i]
                print(f"{i+1:.0f}\t\t{pm.wdia:.2f}\t\t{pm.bwid:.2f}\t\t{pm.bhei:.2f}\t\t{pm.turn:.0f}\t\t{pm.clen:.2f}\t\t{pm.scon:.0f}\t\t{pm.clen*pm.scon:.2f}\t\t{pm.capa:.2f}\t\t{pm.volt:.2f}\t\t{pm.fwid:.2f}\t\t{pm.fhei:.2f}\t\t{pm.cres:.2f}\t\t{pm.cind:.2f}")
        elif prompt == "S":
            break
    ...
    sim = PyLTSpice.SimRunner(output_folder="./output/ltspice",simulator="D:\\ProgramFiles\\LTspice\\LTspice.exe",parallel_sims=5)
    ...
    for i in parameters:
        ...
        femm.mi_addblocklabel((i.bwid/20+i.fwid/20)/2,0)
        femm.mi_selectlabel((i.bwid/10+i.fwid/10)/2,0)
        ...
        prog+=1
        print(f"Finished sim {prog}/{len(parameters)}:\t[{i.wdia:.2f}mm, {i.turn:.0f}tu, {i.curr:.2f}A]")
    ...
    print("Test [xx]\tWdia [mm]\tBwid [mm]\tBhei [mm]\tTurn [xx]\tClen [mm]\tScon [xx]\tTlen [mm]\tCapa [µF]\tVolt [1V]\tFwid [mm]\tFhei [mm]\tCres [mΩ]\tCind [µH]\tCurr [1A]\tFild [1T]\tForc [1N]\tAcce [m/s2]\tSped [m/s]\tEngy [1J]")
    for i in range(len(parameters)):
        pm = parameters[i]
        print(f"{i+1}\t\t{pm.wdia:.2f}\t\t{pm.bwid:.2f}\t\t{pm.bhei:.2f}\t\t{pm.turn:.0f}\t\t{pm.clen:.2f}\t\t{pm.scon:.0f}\t\t{pm.scon*pm.clen:.2f}\t\t{pm.capa:.2f}\t\t{pm.volt:.1f}\t\t{pm.fwid:.2f}\t\t{pm.fhei:.2f}\t\t{pm.cres:.2f}\t\t{pm.cind:.2f}\t\t{pm.curr:.2f}\t\t{pm.fild:.4f}\t\t{pm.forc:.4f}\t\t{pm.acce:.2f}\t\t{pm.sped:.4f}\t\t{pm.engy:.4f}")
    #print(frcs)
...
fullsim([Coilsim(Wdia, Bwid, Bhei, Turn, Clen, Capa, Volt, Scon)
         for Wdia in [awg(i) for i in range(10, 16+1, 2)] # mm (+ AWG)
         for Bwid in [26] # mm
         for Bhei in [5] # mm
         for Turn in [i for i in range(25, 250+1, 25)] # count
         for Clen in [30, 40, 50] # mm
         for Capa in [470, 1000] # uF
         for Volt in [90, 400] # V
...
fullsim([Coilsim(Wdia, Bwid, Bhei, Turn, Clen, Capa, Volt, Scon)
         ...
         for Bwid in [26] # mm
         for Bhei in [5] # mm
         for Turn in [i for i in range(50, 200+1, 50)] # count
         for Clen in [40, 50] # mm
         for Capa in [1000] # uF
         for Volt in [90] # V
         for Scon in [20] # count
         ],
